...
"""Setting up and simulating a 3D flow system with two straight leakage paths,
where CO2 is assessed before and after MICP treatment. Information about the
model and parameters can be found in:
...
def quarter_double_leak():
    ...
    ux1 = 10  # x-gap between aquifer and leakage [m]
    uy1 = 11  # y-gap between aquifer and leakage [m]
    ux2 = 17  # x-gap between aquifer and leakage [m]
    uy2 = 18  # y-gap between aquifer and leakage [m]
    ...
    run_command(
        f"octave --eval 'quarter_double_leak({h}, {ht}, {hl}, {le}, {ux1}, {ux2}, "
        f"{uy1}, {uy2}, {wi}, {x.tolist()}, {y.tolist()}, {z.tolist()})'"
    )
    ...
    # NOW (after 2025.04) this strategy seems not good (after multiplying by 2);
    # the task is to find one.
    injestra.append([2 * 13.7009213840, 2.0e-02, 0.01, 0.00, 0.0])
    injestra.append([2 * 0.4581822278, 2.0e-02, 0.00, 0.00, 0.0])
    injestra.append([2 * 8.6852721663, 0.0e00, 0.00, 0.00, 0.0])
    injestra.append([0.0100000000, 2.0e-02, 0.00, 0.00, 0.0])
    injestra.append([2 * 31.0191624926, 2.0e-02, 0.00, 0.04, 0.0])
    injestra.append([2 * 0.6066256366, 2.0e-02, 0.00, 0.00, 0.0])
    injestra.append([2 * 16.7978691442, 0.0e00, 0.00, 0.00, 0.0])
    injestra.append([0.0100000000, 2.0e-02, 0.00, 0.00, 0.0])
    injestra.append([2 * 13.9589912674, 2.0e-02, 0.00, 0.00, 60.0])
    injestra.append([2 * 1.9373413394, 2.0e-02, 0.00, 0.00, 0.0])
    injestra.append([2 * 15.6607892671, 0.0e00, 0.00, 0.00, 0.0])
    injestra.append([0.0100000000, 2.0e-02, 0.00, 0.00, 0.0])
    injestra.append([2 * 80.2601669260, 2.0e-02, 0.00, 0.04, 60.0])
    injestra.append([2 * 1.0086656673, 2.0e-02, 0.00, 0.00, 0.0])
    injestra.append([2 * 7.3185160841, 0.0e00, 0.00, 0.00, 0.0])
    injestra.append([0.0100000000, 2.0e-02, 0.00, 0.00, 0.0])
    injestra.append([2 * 4.8823307643, 2.0e-02, 0.00, 0.04, 60.0])
    injestra.append([2 * 2.4995251786, 2.0e-02, 0.00, 0.00, 0.0])
    injestra.append([2 * 8.8168603004, 0.0e00, 0.00, 0.00, 0.0])
