...
"""Test the full_single_leak model"""
...
def test_full_single_leak(tmp_path, monkeypatch, mrst_startup):
    """See full_single_leak"""
    monkeypatch.chdir(tmp_path)
    shutil.copytree(mainpth / "full_single_leak", tmp_path, dirs_exist_ok=True)
    for value in [
        "nco2 = 40|nco2 = 1",
        ...
        subprocess.run(
            ["sed", "-i.bak", f"s|{value}|g", "full_single_leak.py"],
            check=True,
        )
    Path("full_single_leak.py.bak").unlink(missing_ok=True)
    ...
    for case, inj, pro in zip(["CO2", "MICP", "CO2MICP"], [3, 16, 3], [24, 12, 24]):
        ...
        assert abs(np.sum(init["PORV"]) - 60002.86) < EPS_PORV
        ...
        with open(name, "r", encoding="utf8") as f:
            lines = f.readlines()
        assert len(lines) == 195638 if case == "MICP" else 172178
