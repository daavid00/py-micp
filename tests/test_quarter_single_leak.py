...
"""Test the quarter_single_leak model"""
...
def test_quarter_single_leak(tmp_path, monkeypatch, mrst_startup):
    """See quarter_single_leak"""
    monkeypatch.chdir(tmp_path)
    shutil.copytree(mainpth / "quarter_single_leak", tmp_path, dirs_exist_ok=True)
    for value in [
        "nco2 = 40|nco2 = 1",
        ...
        subprocess.run(
            ["sed", "-i.bak", f"s|{value}|g", "quarter_single_leak.py"],
            check=True,
        )
    Path("quarter_single_leak.py.bak").unlink(missing_ok=True)
    default = "/Users/dmar/Github/py-micp/MRST/startup.m"
    subprocess.run(
        ["sed", "-i.bak", f"s|{default}|{mrst_startup}|g", "quarter_single_leak.m"],
        check=True,
    )
    Path("quarter_single_leak.m.bak").unlink(missing_ok=True)

    subprocess.run(
        [
            "python3",
            "quarter_single_leak.py",
        ],
        check=True,
    )
    ...
    for case, inj, pro in zip(["CO2", "MICP", "CO2MICP"], [3, 16, 3], [9, 3, 9]):
        ...
        assert abs(np.sum(init["PORV"]) - 15003) < EPS_PORV
        ...
        with open(name, "r", encoding="utf8") as f:
            lines = f.readlines()
        assert len(lines) == 47858 if case == "MICP" else 42133
