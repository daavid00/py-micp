...
"""Test the full_double_leak model"""
...
def test_full_double_leak(tmp_path, monkeypatch, mrst_startup):
    """See full_double_leak"""
    monkeypatch.chdir(tmp_path)
    shutil.copytree(mainpth / "full_double_leak", tmp_path, dirs_exist_ok=True)
    for value in [
        "nco2 = 40|nco2 = 1",
        ...
        subprocess.run(
            ["sed", "-i.bak", f"s|{value}|g", "full_double_leak.py"],
            check=True,
        )
    Path("full_double_leak.py.bak").unlink(missing_ok=True)
    default = "/Users/dmar/Github/py-micp/MRST/startup.m"
    subprocess.run(
        ["sed", "-i.bak", f"s|{default}|{mrst_startup}|g", "full_double_leak.m"],
        check=True,
    )
    Path("full_double_leak.m.bak").unlink(missing_ok=True)
    ...
    for case, inj, pro in zip(["CO2", "MICP", "CO2MICP"], [3, 16, 3], [24, 12, 24]):
        ...
        assert abs(np.sum(init["PORV"]) - 60005.723) < EPS_PORV
        ...
        with open(name, "r", encoding="utf8") as f:
            lines = f.readlines()
        assert len(lines) == 195700 if case == "MICP" else 172235
